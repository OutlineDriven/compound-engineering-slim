...
Core workflow skills like `ce-plan` are deeply chained (`ce-brainstorm` → `ce-plan` → `ce-work`) and orchestrated by autonomous workflows. Rewriting these skills risks breaking the entire workflow for all users simultaneously. There was no mechanism to let users trial new skill versions alongside stable ones.
...
- Autonomous orchestration continues to use stable skills — no modification needed
...
**Simplicity over seamless integration.** Beta skills exist as standalone, manually-invoked skills. They won't be auto-triggered by `ce-brainstorm` handoffs or autonomous workflows without further surgery, which isn't worth the complexity for a trial period.
...
8. Verify autonomous workflows work with the promoted skill
