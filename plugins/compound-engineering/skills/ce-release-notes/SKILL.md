...
**Soft 25-line cap.** If the body exceeds 25 rendered lines, keep the first 25 lines and append `-- N more changes, see full release notes: {url}`. Truncation must be **markdown-fence aware**: count the triple-backtick fence lines that appear in the kept portion. If the count is odd, the cut landed inside an open code fence; close it with a `` ``` `` line on the truncated output before appending the "see more" note, so renderers do not swallow the note or following content.
